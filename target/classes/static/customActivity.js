...
define(function (require) {
    ...
    function requestedInteractionHandler(settings) {
        try {
            ...
            settings.triggers[0].configurationArguments.eventDataConfig.objects.forEach((obj) = > {deFields = deFields.concat(obj.fields.map((fieldName) = > {return obj.dePrefix + fieldName;
        }))
            ;
        })
            ;deFields.forEach((option) = > {$('#select-id-dropdown'
        ).
            append($('<option>', {value: option, text: option}));
        })
            ;$('#select-id').hide();
...
function save() {
    ...
    payload['arguments'].execute.inArguments = [{
        ...
        'recipient': '{{Event.' + eventDefinitionKey + '.\"recipient\"}}',
        'sendStatusTo': '{{Event.' + eventDefinitionKey + '.\"sendStatusTo\"}}',
        'serviceHours': '{{Event.' + eventDefinitionKey + '.\"serviceHours\"}}'
    }];
